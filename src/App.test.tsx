...
describe("Superpowers Progress dashboard", () => {
  ...
  it("opens on a routing home page with all methodology cards", () => {
    renderAt();

    expect(screen.getByRole("heading", { name: "Шпаргалка по методологиям разработки" })).toBeInTheDocument();
  ...
  it("navigates from the home card to the Git page", async () => {
    const user = userEvent.setup();
    renderAt();

    await user.click(screen.getByRole("link", { name: /^git/i }));

    expect(window.location.pathname).toBe("/git");
    expect(screen.getByRole("heading", { name: "Git", level: 1 })).toBeInTheDocument();
  });

  it("opens the Russian Git cheat sheet and updates the selected command", async () => {
    const user = userEvent.setup();
    renderAt("/git");

    expect(screen.getByRole("heading", { name: "Git", level: 1 })).toBeInTheDocument();
    expect(
      screen.getByText("Шесть команд ежедневного цикла, три принципа и типовой feature-flow.")
    ).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: /git commit -m "message"/i }));

    const detail = within(screen.getByRole("complementary", { name: "Выбранная команда Git" }));
    expect(detail.getByRole("heading", { name: "Commit" })).toBeInTheDocument();
    expect(detail.getByText("Создаёт атомарный снимок — точку, в которую можно откатиться.")).toBeInTheDocument();
  });

  it("shows advanced Git commands on the cheat sheet", () => {
    renderAt("/git");

    expect(screen.getByRole("heading", { name: "Продвинутые команды" })).toBeInTheDocument();
    expect(screen.getByText("Interactive Rebase")).toBeInTheDocument();
    expect(screen.getByText("git rebase -i HEAD~N")).toBeInTheDocument();
    expect(screen.getByText("Reflog")).toBeInTheDocument();
    expect(screen.getByText("git reflog")).toBeInTheDocument();
    expect(screen.getByText("Bisect")).toBeInTheDocument();
  });

  it("shows a Git pipeline example on the cheat sheet", () => {
    renderAt("/git");

    expect(screen.getByRole("heading", { name: "Пример Git pipeline" })).toBeInTheDocument();
    expect(screen.getByText("Feature: страница входа")).toBeInTheDocument();
    expect(
      screen.getByText("Как провести изменение от создания ветки до merge в main без сюрпризов.")
    ).toBeInTheDocument();
    expect(screen.getByText("git switch -c feature/login")).toBeInTheDocument();
    expect(screen.getByText('git commit -m "feat: add login form"')).toBeInTheDocument();
  });
});
